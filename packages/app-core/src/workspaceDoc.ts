/**
 * workspaceDoc — the serialized workspace document stored in a cloud
 * notebook's `content` column (#60/#88).
 *
 * Pure module (no fetch, no host environment): serialization/parsing is
 * shared by every surface that talks to the cloud API. Parse failures throw
 * `WorkspaceParseError` with a machine-readable `code`; hosts map codes to
 * their own user-facing copy (the web app maps them to LocalizableError
 * message keys).
 */

/** One open file's serialized notebook. */
...
/** Why a workspace document failed to parse. */
export type WorkspaceParseErrorCode = "notWorkspace" | "noNotebooks";

/**
 * Thrown by `parseWorkspace` when `content` is not a workspace document
 * (`notWorkspace`) or contains no valid notebook files (`noNotebooks`).
 */
export class WorkspaceParseError extends Error {
  readonly code: WorkspaceParseErrorCode;

  constructor(code: WorkspaceParseErrorCode) {
    super(`workspace parse failed: ${code}`);
...
export function parseWorkspace(content: string): ParsedWorkspace {
  ...
  try {
    ...
    throw new WorkspaceParseError("notWorkspace");
  }
  if (!isRecord(parsed)) {
    throw new WorkspaceParseError("notWorkspace");
  }
  ...
  if (files.length === 0) {
    throw new WorkspaceParseError("noNotebooks");
  }
